...
describe("PairingAlgorithmResolver", () => {
  ...
  it("should execute pairing when user is admin", async () => {
    (prisma.pairing.count as jest.Mock)
  ...
  it("should throw ForbiddenException when user is not admin", async () => {
    await expect(
      resolver.executePairingAlgorithm(
        "org-1",
        mockUser({ role: "user", appRole: "user" })
      )
  ...
  it("should handle service errors gracefully", async () => {
    ...
    await expect(
      resolver.executePairingAlgorithm("org-1", mockUser())
